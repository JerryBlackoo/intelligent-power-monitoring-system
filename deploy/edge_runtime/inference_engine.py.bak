...
class AclOmInferenceEngine(InferenceEngine):
    ...
    def _execute(self, input_tensor):
        ...
        try:
            ...
            for idx, buffer in enumerate(output_buffers):
                ...
                ret = acl.rt.memcpy(
                    acl.util.bytes_to_ptr(memoryview(host_buf)),
                    size,
                    buffer,
                    size,
                ...
                outputs.append(self.np.frombuffer(host_buf, dtype=dtype).reshape(shape).copy())
            return outputs[0] if len(outputs) == 1 else outputs
        finally:
            self._destroy_dataset(input_dataset, free_buffers=True)
    ...
    def _host_ptr(self, arr):
        if hasattr(self.acl.util, "bytes_to_ptr"):
            return self.acl.util.bytes_to_ptr(arr.tobytes())
    ...
    def _mem_malloc_normal_policy(self):
        return self._acl_const("ACL_MEM_MALLOC_NORMAL_ONLY", 2)
